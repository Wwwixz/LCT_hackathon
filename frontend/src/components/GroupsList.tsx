...
const profiles = {
  teacher: {
    name: "Вадим Викторович",
    role: "Преподаватель",
    back: "/teacher",
    target: "/students",
  },
  admin: {
    name: "Игнат Мустарович",
    role: "Администратор",
    back: "/admin",
    target: "/admin/group-students",
  },
};

const defaultGroups: GroupEntry[] = [
  { name: "ДВ 102", sub: "2 сессии", students: "26" },
];
...
export default function GroupsList({
  ...
  const filtered = groups.filter((g) =>
    (g.name + " " + g.sub).toLowerCase().includes(q)
  );
  ...
  return (
    <div className="dashboard">
      ...
      <div className="stl-toolbar">
        ...
        <h1 className="stl-title">Группы</h1>
      ...
      <div className="stl-list">
        {filtered.map((g, i) => (
          <div
            ...
            key={g.name}
            onClick={() => { window.location.href = profile.target; }}
          >
            <span className="stl-num">{i + 1}</span>
            <span className="stl-dot" />
            <div className="stl-name">
              <p className="stl-name__title">{g.name}</p>
              <p className="stl-name__sub">{g.sub}</p>
            </div>
            <div className="stl-col stl-col--score">
              <p className="stl-label">Студенты</p>
              <p className="stl-value">{g.students}</p>
            </div>
          </div>
        ))}
        {filtered.length === 0 && (
